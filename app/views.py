...
class ExpenceViewset(viewsets.ModelViewSet):
    ...
    def list(self,request,*args,**kwargs):
        income_list = Report.filters.filter_this_month_expence(year = request.GET.get('year',None),month=request.GET.get('month',None))
        serizlizer = self.get_serializer(income_list, many = True)
        data = {}
        data['items'] = serizlizer.data
        data['expence'] = (Report.filters.tottal_month_report(year = request.GET.get('year',None),month=request.GET.get('month',None),expence=Report.EXPENSE))
        return Response(data,status=status.HTTP_200_OK)
...
class FundViewset(viewsets.ModelViewSet):
    ...
    def list(self,request,*args,**kwargs):
        income_list = Report.filters.filter_this_month_fund(year = request.GET.get('year',None),month=request.GET.get('month',None))
        serizlizer = self.get_serializer(income_list, many = True)
        data = {}
        data['items'] = serizlizer.data
